...
<small>Also in this series: [index](../../) | [the Python notebook](../../python/) | [the browser version](../../wasm/) |
[the other R runner](../) | [source](https://github.com/mdsumner/spilhaus-tracker) |
[the story so far](https://www.hypertidy.org/posts/2026-09-20_road-to-spilhaus/)</small>

The companion to [the Python notebook](../../python/). Same sources, same
...
```{r provenance}
HOBART <- cbind(147.33, -42.88)

## One forward-transform closure per package. The probe is "transform Hobart and get
## finite numbers", because several packages will happily build a CRS object from an
## unknown projection and only fail (or return NA) when asked to use it.
engine_forward <- list(
  gdalraster = function(lonlat, ps) gdalraster::transform_xy(lonlat, "OGC:CRS84", ps),
  PROJ       = function(lonlat, ps) PROJ::proj_trans(lonlat, ps, source = "OGC:CRS84"),
  sf         = function(lonlat, ps) sf::sf_project("OGC:CRS84", ps, lonlat, keep = TRUE),
  terra      = function(lonlat, ps) terra::crds(terra::project(terra::vect(lonlat, crs = "OGC:CRS84"), ps)),
  vapour     = function(lonlat, ps) vapour::vapour_project(lonlat, ps, "OGC:CRS84")
)
engine_inverse <- list(
  gdalraster = function(xy, ps) gdalraster::transform_xy(xy, ps, "OGC:CRS84"),
  PROJ       = function(xy, ps) PROJ::proj_trans(xy, "OGC:CRS84", source = ps),
  sf         = function(xy, ps) sf::sf_project(ps, "OGC:CRS84", xy, keep = TRUE),
  terra      = function(xy, ps) terra::crds(terra::project(terra::vect(xy, crs = ps), "OGC:CRS84")),
  vapour     = function(xy, ps) vapour::vapour_project(xy, "OGC:CRS84", ps)
)

probe <- function(pkg, ps = SPILHAUS) {
  out <- tryCatch({
    xy <- suppressWarnings(matrix(as.numeric(engine_forward[[pkg]](HOBART, ps)), ncol = 2))
    if (nrow(xy) != 1 || !all(is.finite(xy))) stop("transform returned NA/Inf") else "ok"
  }, error = function(e) paste("FAIL:", conditionMessage(e)))
  substr(out, 1, 80)
}
has <- function(pkg) requireNamespace(pkg, quietly = TRUE)
## "installed" is not "loadable": a binary built against another GDAL fails at load time
load_error <- function(pkg) tryCatch({ loadNamespace(pkg); NULL }, error = function(e) conditionMessage(e))
row_for_broken <- function(pkg, msg) data.frame(package = pkg, version = as.character(packageVersion(pkg)),
                                                gdal = "cannot load", proj = "cannot load",
                                                spilhaus = paste("FAIL:", substr(msg, 1, 60)))

rows <- list()

if (has("gdalraster") && !is.null(m <- load_error("gdalraster"))) {
  rows$gdalraster <- row_for_broken("gdalraster", m)
} else if (has("gdalraster")) {
...
if (has("sf") && !is.null(m <- load_error("sf"))) {
  rows$sf <- row_for_broken("sf", m)
} else if (has("sf")) {
...
if (has("terra") && !is.null(m <- load_error("terra"))) {
  rows$terra <- row_for_broken("terra", m)
} else if (has("terra")) {
  rows$terra <- data.frame(
    ...
    spilhaus = probe("terra"))
} else rows$terra <- data.frame(package = "terra", version = NA, gdal = NA, proj = NA, spilhaus = "not installed")

if (has("vapour") && !is.null(m <- load_error("vapour"))) {
  rows$vapour <- row_for_broken("vapour", m)
} else if (has("vapour")) {
...
cli <- c(gdalinfo = cli_version("gdalinfo"),
         ...
         proj = cli_version("proj", character()))  # bare `proj` prints "Rel. x.y.z" to stderr
knitr::kable(data.frame(tool = names(cli), version = unname(cli)), caption = "Command-line tools on PATH")
```

```{r engine}
capable <- provenance$package[provenance$spilhaus == "ok" & provenance$package %in% names(engine_forward)]
RASTER_OK <- has("gdalraster") && is.null(load_error("gdalraster"))
if (length(capable) == 0) {
  cat("No installed package can transform to +proj=spilhaus on this machine.",
      "The table above is the whole result; the document stops here.\n")
  knitr::knit_exit()
}
ENGINE <- capable[1]
cat("Coordinate transforms below use:", ENGINE, "\n")
if (!RASTER_OK) cat("gdalraster is not loadable here, so the raster sections will be skipped.\n")

## forward/inverse constructors for any proj string, from the first capable package
make_forward <- function(proj_string) function(lonlat) engine_forward[[ENGINE]](lonlat, proj_string)
make_inverse <- function(proj_string) function(xy) engine_inverse[[ENGINE]](xy, proj_string)
...
#| eval: !expr exists("w") && exists("segs")
...
```{r wkt}
to_wkt <- function(x) {
  f <- if (RASTER_OK) function(x) gdalraster::srs_to_wkt(x, pretty = TRUE)
       else if (has("sf")) function(x) sf::st_as_text(sf::st_crs(x), pretty = TRUE)
       else if (has("terra")) function(x) terra::crs(x)
       else function(x) stop("no package here can render WKT")
  tryCatch(f(x), error = function(e) paste("(", conditionMessage(e), ")"))
}
...
(Skipped when the raster section was.)
...
#| eval: !expr exists("w") && exists("geom")
